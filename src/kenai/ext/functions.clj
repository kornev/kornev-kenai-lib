...
(defn grouping-id [& exprs]
  (functions/grouping_id (scala-collection/->seq (->col-array exprs))))
...
(link base-64 base64)
(link cbrt cube-root)
(link covar-samp covar)
(link crc-32 crc32)
(link datediff date-diff)
(link dayofmonth day-of-month)
(link dayofweek day-of-week)
(link dayofyear day-of-year)
(link explode explode-outer)
(link expm-1 expm1)
(link log-10 log10)
(link log-1p log1p)
(link log-2 log2)
(link md-5 md5)
(link not !)
(link posexplode posexplode-outer)
(link pow **)
(link sha-1 sha1)
(link sha-2 sha2)
(link signum sign)
(link stddev std)
(link stddev stddev-samp)
(link to-date ->date-col)
(link to-timestamp ->timestamp-col)
(link to-utc-timestamp ->utc-timestamp)
(link unbase-64 unbase64)
(link variance var-samp)
(link weekofyear week-of-year)
(link window time-window)
